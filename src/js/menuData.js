export const menuData = {
    PIZZA: [
      {
        title: 'PIZZA QUATRO STAGIONI',
        subtitle: 'Integer ullamcorper neque eu purus euismo',
        price: '55,68 USD',
      },
      {
        title: 'PIZZA QUATRO STAGIONI',
        subtitle: 'Integer ullamcorper neque eu purus euismo',
        price: '55,68 USD',
      },
      {
        title: 'PIZZA QUATRO STAGIONI',
        subtitle: 'Integer ullamcorper neque eu purus euismo',
        price: '55,68 USD',
      },
      {
        title: 'PIZZA QUATRO STAGIONI',
        subtitle: 'Integer ullamcorper neque eu purus euismo',
        price: '55,68 USD',
      },
      {
        title: 'PIZZA QUATRO STAGIONI',
        subtitle: 'Integer ullamcorper neque eu purus euismo',
        price: '55,68 USD',
      },
      {
        title: 'PIZZA QUATRO STAGIONI',
        subtitle: 'Integer ullamcorper neque eu purus euismo',
        price: '55,68 USD',
      },
    ],
    SOUP: [
      {
        title: 'MINESTRONE',
        subtitle: 'At vero eos et accusamus et iusto odio dignissimos ducimus',
        price: '23,41 USD',
      },
      {
        title: 'MINESTRONE',
        subtitle: 'At vero eos et accusamus et iusto odio dignissimos ducimus',
        price: '23,41 USD',
      },
      {
        title: 'MINESTRONE',
        subtitle: 'At vero eos et accusamus et iusto odio dignissimos ducimus',
        price: '23,41 USD',
      },
      {
        title: 'MINESTRONE',
        subtitle: 'At vero eos et accusamus et iusto odio dignissimos ducimus',
        price: '23,41 USD',
      },
      {
        title: 'MINESTRONE',
        subtitle: 'At vero eos et accusamus et iusto odio dignissimos ducimus',
        price: '23,41 USD',
      },
      {
        title: 'MINESTRONE',
        subtitle: 'At vero eos et accusamus et iusto odio dignissimos ducimus',
        price: '23,41 USD',
      },
      {
        title: 'MINESTRONE',
        subtitle: 'At vero eos et accusamus et iusto odio dignissimos ducimus',
        price: '23,41 USD',
      },
    ],
    PASTA: [
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
      {
        title: 'PASTA ALLA CARBONARA',
        subtitle: 'Itaque earum rerum hic tenetur a sapiente delectus',
        price: '47,12 USD',
      },
    ],
    DESERT: [
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
      {
        title: 'SICILIAN CANNOLI',
        subtitle: 'Nam libero tempore, cum soluta nobis est eligendi optio cumque',
        price: '23,55 USD',
      },
    ],
    WINE: [
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
      {
        title: 'BRUNELLO DI MONTALCINO',
        subtitle: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam',
        price: '76,93 USD',
      },
    ],
    BEER: [
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
      {
        title: 'VIADEI BIRRAI',
        subtitle: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui',
        price: '11,74 USD',
      },
    ],
    DRINKS: [
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
      {
        title: 'LIMONCELLO',
        subtitle: 'Sint occaecati cupiditate non provident, similique sunt in culpa',
        price: '16,44 USD',
      },
    ]
  }